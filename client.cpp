...
#include <atomic>
using namespace std;
atomic<unsigned long long> client_num(0);
#endif
...
bool client::setup_client(benchmark_config *config, abstract_protocol *protocol, object_generator *objgen)
{
    ...
    #ifdef SEQUENCE_KEY
    //sequence_key_num = new atomic<int>(client_num.load(memory_order_relaxed) * config->requests);
    //printf("sequence_key_num = %d\n", sequence_key_num->load(memory_order_relaxed));
    //client_num.fetch_add(1);
    key_num = client_number * (config->requests);
    if(config->update > 0 && config->update < 100){
        int minus_key_range = ((config->requests) * (config->clients)) / config->update;
        key_num -= minus_key_range;  
    }
    client_number += 1;
    #endif
