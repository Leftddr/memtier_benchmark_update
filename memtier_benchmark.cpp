...
static int config_parse_args(int argc, char *argv[], struct benchmark_config *cfg)
{
    ...
    while ((c = getopt_long(argc, argv,
                "s:S:p:P:o:x:DRn:c:t:d:a:u:h", long_options, &option_index)) != -1)
    {
        switch (c) {
                case 'h':
                    return -1;
				...
				case 'u':
                    endptr = NULL;
                    cfg->update = (unsigned int) strtoul(optarg, &endptr, 10);
                    if(cfg->update < 0 || cfg->update > 100){
                        printf("Input Proper update Ratio");
                        exit(0);
                    }
                    if (!endptr || *endptr != '\0') {
